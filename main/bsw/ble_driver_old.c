 ...
 * 
 * ESP32-C6의 BLE 컨트롤러 레지스터를 직접 제어하는 BSW 계층 드라이버 구현입니다.
 * 순수 비트연산을 사용하여 BLE 통신 기능을 제공합니다.
 * 
...
} bsw_ble_driver_context_t;

static bsw_ble_driver_context_t g_bsw_ble_ctx = {0};
...
// 내부 함수 선언 (비트연산 방식)
static bool bsw_ble_controller_init_bitwise(void);
static bool bsw_ble_setup_advertising_bitwise(void);
static void bsw_ble_delay_us(uint32_t us);
...
// BLE 드라이버 초기화 (비트연산 방식)
bool ble_driver_init_bitwise(const char* device_name, ble_event_callback_t callback, void* user_data)
{
    if (g_bsw_ble_ctx.initialized) {
        return true;  // 이미 초기화됨
    }
    ...
    bsw_ble_delay_us(1000);  // 1ms 대기
    BSW_BLE_REG_CLEAR_BIT(BSW_BLE_CTRL_CONFIG, BSW_BLE_CTRL_RESET_BIT);
    
    // BLE 컨트롤러 활성화
    if (!bsw_ble_controller_init_bitwise()) {
        return false;
    }
    
    // 컨텍스트 초기화
    strncpy(g_bsw_ble_ctx.device_name, device_name, sizeof(g_bsw_ble_ctx.device_name) - 1);
    g_bsw_ble_ctx.device_name[sizeof(g_bsw_ble_ctx.device_name) - 1] = '\0';
    g_bsw_ble_ctx.event_callback = callback;
    g_bsw_ble_ctx.user_data = user_data;
    g_bsw_ble_ctx.controller_state = 0;
    g_bsw_ble_ctx.adv_config = 0;
    g_bsw_ble_ctx.adv_data_len = 0;
    
    g_bsw_ble_ctx.initialized = true;
    return true;
}
