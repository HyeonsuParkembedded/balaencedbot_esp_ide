...
esp_err_t servo_standup_init(servo_standup_t* servo, bsw_gpio_num_t pin, pwm_channel_t channel, int extend_angle, int retract_angle) {
    ...
    // BSW PWM 드라이버 초기화
    esp_err_t ret = pwm_servo_init(pin, channel);
    if (ret != ESP_OK) {
        BSW_LOGE(SERVO_TAG, "Failed to initialize servo PWM channel");
        return ret;
    }
    ...
    switch (servo->state) {
        case STANDUP_EXTENDING:
            if (elapsed_time == 0 || servo->current_angle != servo->extended_angle) {
                ...
                BSW_LOGI(SERVO_TAG, "Extending servo to %d degrees", servo->extended_angle);
            }
            if (elapsed_time >= servo->extend_duration) {
                servo->state = STANDUP_PUSHING;
                servo->state_start_time = current_time;
                BSW_LOGI(SERVO_TAG, "Holding position for push");
            }
            break;
        ...
        case STANDUP_PUSHING:
            if (elapsed_time >= servo->push_duration) {
                ...
                BSW_LOGI(SERVO_TAG, "Retracting servo to %d degrees", servo->retracted_angle);
            }
            break;
        ...
        case STANDUP_RETRACTING:
            if (elapsed_time >= servo->retract_duration) {
                ...
                servo->state_start_time = current_time;
                BSW_LOGI(SERVO_TAG, "Standup sequence complete");
            }
            break;
        ...
        case STANDUP_COMPLETE:
            if (elapsed_time >= 500) { // Wait 500ms before allowing next standup
                servo->state = STANDUP_IDLE;
                servo->standup_in_progress = false;
                BSW_LOGI(SERVO_TAG, "Ready for next standup");
            }
            break;
...
void servo_standup_reset(servo_standup_t* servo) {
    ...
    servo->standup_requested = false;
    servo_set_angle(servo, servo->retracted_angle);
    BSW_LOGI(SERVO_TAG, "Servo standup reset");
}
 ...
 * 로봇의 무게나 환경에 따라 최적화된 타이밍을 적용할 수 있습니다.
 * 
 * @param servo 서보 기립 구조체 포인터
